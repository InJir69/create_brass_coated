...
public class BrassHarvesterRenderer extends SafeTileEntityRenderer<BrassHarvesterTileEntity> {

	public BrassHarvesterRenderer(BlockEntityRendererProvider.Context context) {
	}

	@Override
	protected void renderSafe(BrassHarvesterTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		int light, int overlay) {
		BlockState blockState = te.getBlockState();
		SuperByteBuffer superBuffer = CachedBufferer.partial(BrassPartials.BRASS_HARVESTER_BLADE, blockState);
		transform(te.getLevel(), blockState.getValue(BrassHarvesterBlock.FACING), superBuffer,
				te.getAnimatedSpeed());
