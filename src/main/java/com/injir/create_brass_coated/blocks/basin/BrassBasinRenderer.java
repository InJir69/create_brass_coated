...
public class BrassBasinRenderer extends SmartTileEntityRenderer<BrassBasinTileEntity> {

	public BrassBasinRenderer(BlockEntityRendererProvider.Context context) {
		super(context);
	}

	@Override
	protected void renderSafe(BrassBasinTileEntity basin, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		...
		boolean outToBasin = basin.getLevel()
			.getBlockState(basin.getBlockPos()
				.relative(direction))
			.getBlock() instanceof BrassBasinBlock;

		for (IntAttached<ItemStack> intAttached : basin.visualizedOutputItems) {
			float progress = 1 - (intAttached.getFirst() - partialTicks) / BrassBasinTileEntity.OUTPUT_ANIMATION_TIME;
	...
	protected float renderFluids(BrassBasinTileEntity basin, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		int light, int overlay) {
