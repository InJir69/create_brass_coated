...
public class BrassSawRenderer extends SafeTileEntityRenderer<BrassSawTileEntity> {

	public BrassSawRenderer(BlockEntityRendererProvider.Context context) {
	}

	@Override
	protected void renderSafe(BrassSawTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light,
		int overlay) {
	...
	protected void renderBlade(BrassSawTileEntity te, PoseStack ms, MultiBufferSource buffer, int light) {
		BlockState blockState = te.getBlockState();
		PartialModel partial;
		float speed = te.getSpeed();
		boolean rotate = false;

		if (BrassSawBlock.isHorizontal(blockState)) {
			if (speed > 0) {
			...
			if (blockState.getValue(BrassSawBlock.AXIS_ALONG_FIRST_COORDINATE))
				rotate = true;
		}
	...
	protected void renderShaft(BrassSawTileEntity te, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
		KineticTileEntityRenderer.renderRotatingBuffer(te, getRotatedModel(te), ms,
			buffer.getBuffer(RenderType.solid()), light);
	}

	protected void renderItems(BrassSawTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		int light, int overlay) {
		boolean processingMode = te.getBlockState()
			.getValue(BrassSawBlock.FACING) == Direction.UP;
		if (processingMode && !te.inventory.isEmpty()) {
			boolean alongZ = !te.getBlockState()
				.getValue(BrassSawBlock.AXIS_ALONG_FIRST_COORDINATE);
			ms.pushPose();
	...
	public static void renderInContraption(MovementContext context, VirtualRenderWorld renderWorld,
		...
		Vec3 facingVec = Vec3.atLowerCornerOf(context.state.getValue(BrassSawBlock.FACING)
			.getNormal());
		...
		if (BrassSawBlock.isHorizontal(state)) {
			if (shouldAnimate)
		...
		if (!BrassSawBlock.isHorizontal(state)) {
			superBuffer.rotateZ(state.getValue(BrassSawBlock.AXIS_ALONG_FIRST_COORDINATE) ? 90 : 0);
		}
