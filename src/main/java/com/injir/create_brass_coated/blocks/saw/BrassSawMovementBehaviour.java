...
public class BrassSawMovementBehaviour extends BlockBreakingMovementBehaviour {

	@Override
	public boolean isActive(MovementContext context) {
		return !VecHelper.isVecPointingTowards(context.relativeMotion, context.state.getValue(BrassSawBlock.FACING)
			.getOpposite());
	}

	@Override
	public Vec3 getActiveAreaOffset(MovementContext context) {
		return Vec3.atLowerCornerOf(context.state.getValue(BrassSawBlock.FACING)
			.getNormal())
			.scale(.65f);
	}
	...
	public boolean canBreak(Level world, BlockPos breakingPos, BlockState state) {
		return super.canBreak(world, breakingPos, state) && BrassSawTileEntity.isSawable(state);
	}
	...
	public void renderInContraption(MovementContext context, VirtualRenderWorld renderWorld,
		...
		BrassSawRenderer.renderInContraption(context, renderWorld, matrices, buffer);
	}
	...
	protected DamageSource getDamageSource() {
		return BrassSawBlock.damageSourceBrassSaw;
	}
}
