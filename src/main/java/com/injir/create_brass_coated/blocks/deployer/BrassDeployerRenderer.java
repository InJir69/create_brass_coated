...
public class BrassDeployerRenderer extends SafeTileEntityRenderer<BrassDeployerTileEntity> {

	public BrassDeployerRenderer(BlockEntityRendererProvider.Context context) {
	}

	@Override
	protected void renderSafe(BrassDeployerTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer,
	...
	protected void renderItem(BrassDeployerTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		int light, int overlay) {
		...
		boolean punching = te.mode == BrassDeployerTileEntity.Mode.PUNCH;
	...
	protected void renderComponents(BrassDeployerTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		int light, int overlay) {
		...
		SuperByteBuffer pole = CachedBufferer.partial(BrassPartials.BRASS_DEPLOYER_POLE, blockState);
	...
	protected Vec3 getHandOffset(BrassDeployerTileEntity te, float partialTicks, BlockState blockState) {
	...
	static PartialModel getHandPose(Mode mode) {
		return mode == Mode.PUNCH ? AllBlockPartials.DEPLOYER_HAND_PUNCHING : AllBlockPartials.DEPLOYER_HAND_POINTING;
	}
