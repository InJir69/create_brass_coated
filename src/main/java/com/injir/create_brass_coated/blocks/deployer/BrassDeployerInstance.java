...
public class BrassDeployerInstance extends ShaftInstance implements DynamicInstance, TickableInstance {

    final BrassDeployerTileEntity tile;
    ...
    public BrassDeployerInstance(MaterialManager dispatcher, KineticTileEntity tile) {
        super(dispatcher, tile);

        this.tile = (BrassDeployerTileEntity) super.blockEntity;
        ...
        pole = getOrientedMaterial().getModel(BrassPartials.BRASS_DEPLOYER_POLE, blockState).createInstance();
	...
	private float getProgress(float partialTicks) {
        if (tile.state == BrassDeployerTileEntity.State.EXPANDING) {
			float f = 1 - (tile.timer - partialTicks * tile.getTimerSpeed()) / 1000f;
			if (tile.fistBump)
				f *= f;
			return f;
		}
        if (tile.state == BrassDeployerTileEntity.State.RETRACTING)
