...
public class BrassEjectorRenderer extends KineticTileEntityRenderer {

	static final Vec3 pivot = VecHelper.voxelSpace(0, 11.25, 0.75);

	public BrassEjectorRenderer(BlockEntityRendererProvider.Context context) {
		super(context);
	}
	...
	protected void renderSafe(KineticTileEntity te, float partialTicks, PoseStack ms, MultiBufferSource buffer,
		...
		BrassEjectorTileEntity ejector = (BrassEjectorTileEntity) te;
		VertexConsumer vertexBuilder = buffer.getBuffer(RenderType.solid());
		float lidProgress = ((BrassEjectorTileEntity) te).getLidProgress(partialTicks);
		float angle = lidProgress * 70;

		if (!Backend.canUseInstancing(te.getLevel())) {
			SuperByteBuffer model = CachedBufferer.partial(BrassPartials.BRASS_EJECTOR_TOP, te.getBlockState());
		...
		BrassDepotBehaviour behaviour = te.getBehaviour(BrassDepotBehaviour.TYPE);
		...
		msr.centre()
			.rotateY(-180 - AngleHelper.horizontalAngle(te.getBlockState()
				.getValue(BrassEjectorBlock.HORIZONTAL_FACING)))
			.unCentre();
		BrassDepotRenderer.renderItemsOf(te, partialTicks, ms, buffer, light, overlay, behaviour);
		ms.popPose();
	}
