...
public class BrassSharedDepotBlockMethods {

	protected static BrassDepotBehaviour get(BlockGetter worldIn, BlockPos pos) {
		return TileEntityBehaviour.get(worldIn, pos, BrassDepotBehaviour.TYPE);
	}
	...
	public static InteractionResult onUse(BlockState state, Level world, BlockPos pos, Player player,
		...
		BrassDepotBehaviour behaviour = get(world, pos);
		if (behaviour == null)
			return InteractionResult.PASS;
	...
	public static int getComparatorInputOverride(BlockState blockState, Level worldIn, BlockPos pos) {
		BrassDepotBehaviour depotBehaviour = get(worldIn, pos);
