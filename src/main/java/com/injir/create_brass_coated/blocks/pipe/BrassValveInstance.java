...
public class BrassValveInstance extends ShaftInstance implements DynamicInstance {

	private final BrassValveTileEntity tile;
    ...
    public BrassValveInstance(MaterialManager dispatcher, KineticTileEntity tile) {
        super(dispatcher, tile);
		this.tile = (BrassValveTileEntity) tile;

        Direction facing = blockState.getValue(BrassValveBlock.FACING);

        yRot = AngleHelper.horizontalAngle(facing);
        xRot = facing == Direction.UP ? 0 : facing == Direction.DOWN ? 180 : 90;

        Direction.Axis pipeAxis = BrassValveBlock.getPipeAxis(blockState);
