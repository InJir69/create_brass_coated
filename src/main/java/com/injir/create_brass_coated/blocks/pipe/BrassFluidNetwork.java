...
public class BrassFluidNetwork {
	...
	Set<Pair<BlockFace, BrassPipeConnection>> frontier;
	Set<BlockPos> visited;
	FluidStack fluid;
	List<Pair<BlockFace, LazyOptional<IFluidHandler>>> targets;
	Map<BlockPos, WeakReference<BrassFluidTransportBehaviour>> cache;

	public BrassFluidNetwork(Level world, BlockFace location, Supplier<LazyOptional<IFluidHandler>> sourceSupplier) {
		this.world = world;
		this.start = location;
		...
		for (int cycle = 0; cycle < CYCLES_PER_TICK; cycle++) {
			...
			for (Iterator<BlockFace> iterator = queued.iterator(); iterator.hasNext();) {
				...
				if (!isPresent(blockFace))
					continue;
				BrassPipeConnection pipeConnection = get(blockFace);
...
//			drawDebugOutlines();
			...
			for (Iterator<Pair<BlockFace, BrassPipeConnection>> iterator = frontier.iterator(); iterator.hasNext();) {
				Pair<BlockFace, BrassPipeConnection> pair = iterator.next();
				BlockFace blockFace = pair.getFirst();
				BrassPipeConnection pipeConnection = pair.getSecond();
				...
				for (Direction side : Iterate.directions) {
					...
					BrassPipeConnection adjacent = get(adjacentLocation);
					if (adjacent == null)
						continue;
					if (!adjacent.hasFlow()) {
...
//		drawDebugOutlines();
		...
		for (Pair<BlockFace, LazyOptional<IFluidHandler>> pair : targets) {
			if (pair.getSecond()
				...
				continue;
			BrassPipeConnection pipeConnection = get(pair.getFirst());
			if (pipeConnection == null)
				continue;
...
//	}
	...
	@Nullable
	private BrassPipeConnection get(BlockFace location) {
		BlockPos pos = location.getPos();
		BrassFluidTransportBehaviour fluidTransfer = getFluidTransfer(pos);
	...
	@Nullable
	private BrassFluidTransportBehaviour getFluidTransfer(BlockPos pos) {
		WeakReference<BrassFluidTransportBehaviour> weakReference = cache.get(pos);
		BrassFluidTransportBehaviour behaviour = weakReference != null ? weakReference.get() : null;
		if (behaviour != null && behaviour.tileEntity.isRemoved())
			behaviour = null;
		if (behaviour == null) {
			behaviour = TileEntityBehaviour.get(world, pos, BrassFluidTransportBehaviour.TYPE);
			if (behaviour != null)
